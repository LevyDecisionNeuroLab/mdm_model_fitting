...
function [info,p] = fit_ambigOnly_model(choice,vF,vA,pF,pA,AL,model,b0,base,optimizer)

% If multiple model fits requested, loop and pack 
if iscell(model)
   for i = 1:length(model)
      [info(i),p{i}] = fit_ambigOnly_model(choice,vF,vA,pF,pA,AL,model{i},b0,base,optimizer);
   end
   return;
end
...
% try multiple starting positions
    for i = 1 : size(b0,1)
        ...
        if i == 1 || (i ~=1 && LL > info.LL)% first iteration; and if a later iteration renders larger likelihood, replace info
            ...
            p = choice_prob_ambigOnly(base,vF,vA,pF,pA,AL,b,model);
...
elseif strcmp(optimizer, 'bads')
    ...
    lb = [-100 -10]; % lower bound for slope, beta, and alpha
    ub = [100 10]; % upper bound for slope, beta, and alpha
...
function sumerr = local_negLL(beta,choice,vF,vA,pF,pA,AL,model,base)
    ...
    p = choice_prob_ambigOnly(base,vF,vA,pF,pA,AL,beta,model); 
